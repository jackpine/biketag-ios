...
class SpotTests: XCTestCase {
  ...
  func testFetchCurrentSpot() {
    ...
    Spot.fetchCurrentSpots(
      location: Spot.lucileSpot.location!, callback: fulfillExpectation,
      errorCallback: failExpectation)
  ...
  func testCreateNewSpot() {
    ...
    Spot.createNewSpot(
      image: image, game: Game(id: 1), location: location, callback: fulfillExpectation,
      errorCallback: failExpectation)
