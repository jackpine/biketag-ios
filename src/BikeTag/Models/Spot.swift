...
class Spot: NSObject {
    ...
    class func fetchCurrentSpots(location: CLLocation, callback: @escaping ([Spot]) -> Void, errorCallback: @escaping (Error) -> Void) {
        let callbackWithBuiltSpots = { (parsedSpots: [ParsedSpot]) -> Void in
            let spots = parsedSpots.map { Spot(parsedSpot: $0) }
            callback(spots)
        }

        Config.spotsService.fetchCurrentSpots(location: location, successCallback: callbackWithBuiltSpots, errorCallback: errorCallback)
    }

    class func createNewSpot(image: UIImage, game: Game, location: CLLocation, callback: @escaping (Spot) -> Void, errorCallback: @escaping (Error) -> Void) {
        ...
        Config.spotsService.postNewSpot(spot: newSpot, callback: callbackWithBuiltSpot, errorCallback: errorCallback)
    }
