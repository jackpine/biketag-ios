...
class SpotsService: ApiService {
  func fetchCurrentSpots(
    location: CLLocation, successCallback: @escaping ([ParsedSpot]) -> Void,
    ...
    let handleResponseAttributes = { (responseAttributes: [String: Any]) -> Void in
      ...
      let parsedSpots = (spotsAttributes["spots"] as! [[String: Any]]).map {
        spotAttributes -> ParsedSpot in
    ...
    request(
      .get, path: "games/current_spots.json", parameters: parameters,
      handleResponseAttributes: handleResponseAttributes, errorCallback: errorCallback)
  }

  func postNewSpot(
    spot: Spot, callback: @escaping (ParsedSpot) -> Void, errorCallback: @escaping (Error) -> Void
  ) {
    // TODO: use param struct
    ...
    spotParametersForLogging["image_data"] =
      "\(spot.base64ImageData().lengthOfBytes(using: .utf8)) bytes"
    ...
    request(
      .post, path: "spots.json", parameters: parameters,
      handleResponseAttributes: handleResponseAttributes, errorCallback: errorCallback)
  }

  func postSpotGuess(
    guess: Guess, callback: @escaping (Guess) -> Void, errorCallback: @escaping (Error) -> Void
  ) {
    // TODO: use param struct
    let parameters = [
      "guess": [
    ...
    request(
      .post, path: "guesses.json", parameters: parameters,
      handleResponseAttributes: handleResponseAttributes, errorCallback: errorCallback)
  }
