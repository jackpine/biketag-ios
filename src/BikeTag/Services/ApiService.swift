...
class ApiService {
  ...
  func unauthenticatedRequest(
    _ method: HTTPMethod, path: String, parameters: Parameters?,
    handleResponseAttributes: @escaping ([String: Any]) -> Void,
    errorCallback: @escaping (Error) -> Void
  ) {
    request(
      method, path: path, parameters: parameters,
  ...
  func request(
    _ method: HTTPMethod, path: String, parameters: Parameters?,
    handleResponseAttributes: @escaping ([String: Any]) -> Void,
    errorCallback: @escaping (Error) -> Void, isAuthenticated: Bool = true
  ) {
        ...
        case let .success(result):
          guard let responseAttributes = result as? [String: Any] else {
            ...
            return errorCallback(
              APIError.clientError(description: "unprocessable service response"))
          }
