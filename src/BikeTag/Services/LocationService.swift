...
class LocationService: NSObject, CLLocationManagerDelegate {
  ...
  func waitForLocation(
    onSuccess successCallback: @escaping (CLLocation) -> Void,
    onTimeout timeoutCallback: @escaping () -> Void
  ) {
  ...
  func startTrackingLocation(onDenied deniedCallback: () -> Void) {
    ...
    @unknown default:
      assertionFailure(
        "unknown CLLocationManager.authorizationStatus: \(CLLocationManager.authorizationStatus().rawValue)"
      )
    }
  }
  ...
  func locationManager(
    _: CLLocationManager,
    didChangeAuthorization _: CLAuthorizationStatus
  ) {
    startTrackingLocation(onDenied: {
      Logger.error("sneaky user disabled location authorization status.")
    })
  }
