...
class CameraViewController: BaseViewController, CLLocationManagerDelegate {
  ...
  lazy var bottomSection: UIView = {
    ...
    bottomSection.label.text = NSLocalizedString(
      "Don't forget to include your bike in the shot!", comment: "label text overlaying camera view"
    )
  ...
  func captureImage(callback: @escaping (Data, CLLocation) -> Void) {
    ...
    stillImageOutput.captureStillImageAsynchronously(from: videoConnection) {
      [weak self] imageDataSampleBuffer, _ -> Void in
      ...
      let finalImage = UIImage(
        cgImage: finalCgImage, scale: 1.0, orientation: image.imageOrientation)
