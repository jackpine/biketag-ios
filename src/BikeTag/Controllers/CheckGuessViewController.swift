...
protocol CheckGuessDelegate: AnyObject {
  func checkedGuess(
    _ checkGuessViewController: CheckGuessViewController, didGuessCorrect guess: Guess)
  func checkedGuess(
    _ checkGuessViewController: CheckGuessViewController, didGuessWrong guess: Guess)
}
...
class CheckGuessViewController: BaseViewController {
  ...
  class func fromStoryboard() -> Self {
    ...
    guard
      let vc = storyboard.instantiateViewController(withIdentifier: "CheckGuessViewController")
        as? Self
    else {
  ...
  func submitGuessToServer() {
    ...
    Config.spotsService.postSpotGuess(
      guess: guess!, callback: handleGuessResponse, errorCallback: displayErrorAlert)
  }

  func animateProgress() {
    let progressMessages = [
      "Hmmm...", "O.K.", "...maybe", "Well, actually...", "Ummmm...", "Hold on.",
    ]

    UIView.animate(
      withDuration: 1.0, delay: 1.0,
