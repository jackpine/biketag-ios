...
protocol GuessCreationDelegate: AnyObject {
  ...
  func guessCreation(
    _ newGuessVC: GuessSpotViewController, didApproveImageData imageData: Data,
    location: CLLocation, errorCallback: @escaping (Error) -> Void)
}
...
class GuessSpotViewController: CameraViewController {
  ...
  public class func fromStoryboard(spot _: Spot) -> GuessSpotViewController {
    ...
    guard
      let vc = storyboard.instantiateViewController(withIdentifier: "guessSpotViewController")
        as? GuessSpotViewController
    else {
  ...
  override func didTapCaptureButton() {
    ...
    captureImage { [weak self] capturedImageData, location in
      ...
      self.guessCreationDelegate?.guessCreation(
...
extension GuessSpotViewController: ApprovalDelegate {
  var approvalButtonText: String {
    NSLocalizedString(
      "Claim this Spot! ", comment: "primary button, confirm to upload snapped photo")
  }
  ...
  func approvalView(
    ...
    guessCreationDelegate?.guessCreation(
