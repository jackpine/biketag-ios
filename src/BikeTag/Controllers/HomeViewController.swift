...
class HomeViewController: BaseViewController, UIScrollViewDelegate, UITableViewDelegate,
  UITableViewDataSource
{
  ...
  func renderScore() {
    ...
    if let newSpotCostLabel = newSpotCostLabel {
      if currentUserScore >= Spot.newSpotCost {
        newSpotCostLabel.text =
          "This costs \(User.currencyUnit)\(Spot.newSpotCost) of your \(User.currencyUnit)\(currentUserScore)."
        newSpotButton.isEnabled = true
      } else {
        newSpotCostLabel.text =
          "You need at least \(User.currencyUnit)\(Spot.newSpotCost - currentUserScore) more to add a spot."
  ...
  func didTapScoreButton() {
    ...
    let newSpotAction = UIAlertAction(
      title: "\(User.currencyUnit)\(Spot.newSpotCost) to post your own spot", style: .default
    ) { [weak self] _ in
  ...
  override func viewDidLoad() {
    ...
    refreshControl.addTarget(
      self, action: #selector(refreshControlPulled(sender:)), for: .valueChanged)
  ...
  func addShadowBehindStatusBar() {
    ...
    let topShadow = GradientView(
      from: UIColor.black.withAlphaComponent(0.3),
      to: .clear)
  ...
  func startTrackingLocation() {
    let showLocationServicesDisabledAlert = {
      let alertController = UIAlertController(
        ...
        message:
          "In order to get spots near you, please open this app's settings and set location access to 'While Using the App'.",
        preferredStyle: .alert
      )
  ...
  func ensureLocation(onSuccess successCallback: @escaping (CLLocation) -> Void) {
    ...
    locationService.waitForLocation(
  ...
  func fetchCurrentSpots(near: CLLocation, completion: @escaping (Error?) -> Void = { _ in }) {
    ...
    Spot.fetchCurrentSpots(
      location: near, callback: setCurrentSpots, errorCallback: displayErrorAlert)
  }
  ...
  func fetchCurrentUser() {
    ...
    Config.usersService.fetchUser(
      userId: Config.currentUserId, successCallback: updateCurrentUser,
      errorCallback: displayErrorAlert)
  }
  ...
  func scrollViewWillEndDragging(
    _: UIScrollView, withVelocity _: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>
  ) {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    guard
      let cell: SpotViewCell = tableView.dequeueReusableCell(
        withIdentifier: SpotViewCell.reuseIdentifier) as? SpotViewCell
    else {
...
extension HomeViewController: SpotViewDelegate {
  func spotViewDidTapGuessSpot(_ spot: Spot) {
    ...
    guessSpotVC.navigationItem.leftBarButtonItem = UIBarButtonItem(
      barButtonSystemItem: .stop,
      target: self,
