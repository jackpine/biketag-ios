...
protocol SpotCreationDelegate: AnyObject {
    ...
    func spotCreation(_ newSpotVC: NewSpotViewController, didApproveImageData imageData: Data, location: CLLocation, errorCallback: @escaping (Error) -> Void)
}
...
class NewSpotViewController: CameraViewController {
    ...
    override func didTapCaptureButton() {
        ...
        captureImage { [weak self] capturedImageData, location in
            ...
            self.spotCreationDelegate?.spotCreation(self, didCaptureImageData: imageData, location: location)
...
extension NewSpotViewController: ApprovalDelegate {
    var approvalButtonText: String {
        NSLocalizedString("Post this Spot! ", comment: "primary button, confirm to upload snapped photo")
    }
    ...
    func approvalView(_: ApprovalViewController, didApproveImageData imageData: Data, location: CLLocation, errorCallback: @escaping (Error) -> Void) {
        spotCreationDelegate?.spotCreation(self, didApproveImageData: imageData, location: location, errorCallback: errorCallback)
    }
}
