...
protocol ApprovalDelegate: AnyObject {
  ...
  func approvalView(
    _ approvalViewController: ApprovalViewController, didApproveImageData imageData: Data,
...
class ApprovalViewController: BaseViewController {
  ...
  lazy var bottomSection: UIView = {
    ...
    bottomSection.label.text = NSLocalizedString(
      "Everything looking good? 😎", comment: "label text overlaying camera view")
  ...
  func approveImage() {
    ...
    approvalDelegate?.approvalView(
      self, didApproveImageData: outputImageData, location: location,
      errorCallback: presentAlert(error:))
  }
}
