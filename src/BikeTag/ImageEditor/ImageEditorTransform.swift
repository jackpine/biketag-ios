...
public class ImageEditorTransform: NSObject {
  ...
  public func affineTransform(viewSize: CGSize) -> CGAffineTransform {
    ...
    let transform = CGAffineTransform.identity.translate(translation).rotated(by: rotationRadians)
  ...
  public func normalize(srcImageSizePixels: CGSize) -> ImageEditorTransform {
    ...
    for viewCorner in [
      ...
      let naiveViewCornerInCanvas = viewCorner.minus(viewBounds.center).applyingInverse(
    ...
    let naiveImageFrameCanvas = ImageEditorCanvasView.imageFrame(
      forViewSize: viewBounds.size, imageSize: srcImageSizePixels, transform: naiveTransform)
    let naiveImageSizeCanvas = CGPoint(
      x: naiveImageFrameCanvas.width, y: naiveImageFrameCanvas.height)
    ...
    let maxTranslationCanvas = naiveImageSizeCanvas.minus(naiveViewportSizeCanvas).scaled(by: 0.5)
      .max(.zero)
    ...
    let clampedTranslationInCanvas = translationInCanvas.min(maxTranslationCanvas).max(
      maxTranslationCanvas.inverse())
    let clampedTranslationInView = clampedTranslationInCanvas.applying(naiveAffineTransform)
    let unitTranslation = clampedTranslationInView.toUnitCoordinates(
      viewBounds: viewBounds, shouldClamp: false)
  ...
  override public func isEqual(_ object: Any?) -> Bool {
    ...
    return
      (outputSizePixels == other.outputSizePixels && unitTranslation == other.unitTranslation
      && rotationRadians == other.rotationRadians && scaling == other.scaling
      && isFlipped == other.isFlipped)
  }

  override public var hash: Int {
    return
      (outputSizePixels.width.hashValue ^ outputSizePixels.height.hashValue
      ^ unitTranslation.x.hashValue ^ unitTranslation.y.hashValue ^ rotationRadians.hashValue
      ^ scaling.hashValue ^ isFlipped.hashValue)
  }
