...
public protocol ImageEditorViewDelegate: AnyObject {
  func imageEditor(
    presentFullScreenView viewController: UIViewController,
    isTransparent: Bool)
...
public class ImageEditorView: UIView {
  ...
  public func configureSubviews() {
    ...
    let moveTextGestureRecognizer = ImageEditorPanGestureRecognizer(
      target: self, action: #selector(handleMoveTextGesture(_:)))
  ...
  @objc func didTapBrush(sender _: UIButton) {
    ...
    let brushView = ImageEditorBrushViewController(
      delegate: self, model: model, currentColor: currentColor)
    delegate?.imageEditor(
      presentFullScreenView: brushView,
      isTransparent: false)
  }
  ...
  private func createNewTextItem() {
    ...
    let imageFrame = ImageEditorCanvasView.imageFrame(
      forViewSize: viewSize, imageSize: imageSize,
  ...
  public func handlePinchGesture(_ gestureRecognizer: ImageEditorPinchGestureRecognizer) {
    ...
    case .changed, .ended:
      ...
      let newScaling =
        (textItem.scaling * gestureRecognizer.pinchStateLast.distance
      ...
      let newRotationRadians =
        textItem.rotationRadians + gestureRecognizer.pinchStateLast.angleRadians
        - gestureRecognizer.pinchStateStart.angleRadians

      let newItem = textItem.copy(unitCenter: unitCenter).copy(
        scaling: newScaling,
  ...
  private func textLayer(forLocation locationInView: CGPoint) -> EditorTextLayer? {
    ...
    let locationInCanvas = locationInView.minus(viewBounds.center).applyingInverse(affineTransform)
  ...
  private func edit(textItem: ImageEditorTextItem, isNewItem: Bool) {
    ...
    delegate?.imageEditor(
      presentFullScreenView: textEditor,
      isTransparent: false)
  }
  ...
  private func presentCropTool() {
    ...
    let previewTransform = ImageEditorTransform.defaultTransform(
      srcImageSizePixels: model.srcImageSizePixels)
    guard
      let previewImage = ImageEditorCanvasView.renderForOutput(
        model: model, transform: previewTransform)
    else {
      assertionFailure("Couldn't generate preview image.")
      return
    }

    let cropTool = ImageEditorCropViewController(
      delegate: self, model: model, srcImage: srcImage, previewImage: previewImage)
    delegate?.imageEditor(
      presentFullScreenView: cropTool,
      isTransparent: false)
  }
}
