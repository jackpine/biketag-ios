...
public class ImageEditorCanvasView: UIView {
  ...
  public class func updateContentLayout(
    transform: ImageEditorTransform,
    contentView: UIView
  ) -> [NSLayoutConstraint] {
    guard let superview = contentView.superview else {
      assertionFailure("Content view has no superview.")
      return []
    }
    let outputSizePixels = transform.outputSizePixels

    let aspectRatio = outputSizePixels
    var constraints = superview.applyScaleAspectFitLayout(
      subview: contentView, aspectRatio: aspectRatio.width / aspectRatio.height)
  ...
  public class func loadSrcImage(model: ImageEditorModel) -> UIImage? {
    ...
    do {
      ...
      assertionFailure("Couldn't parse srcImageUrl")
      return nil
    }
    ...
    guard let srcImage = UIImage(data: srcImageData, scale: 1.0) else {
      assertionFailure("Couldn't load background image.")
      return nil
    }
  ...
  private func applyTransform() {
    ...
    contentView.layer.setAffineTransform(
      model.currentTransform().affineTransform(viewSize: viewSize))
  }
  ...
  public class func updateImageLayer(
    imageLayer: CALayer, viewSize: CGSize, imageSize: CGSize, transform: ImageEditorTransform
  ) {
  ...
  public class func imageFrame(
    forViewSize viewSize: CGSize, imageSize: CGSize, transform: ImageEditorTransform
  ) -> CGRect {
    guard viewSize.width > 0, viewSize.height > 0 else {
      assertionFailure("Invalid viewSize")
      return .zero
    }
    guard imageSize.width > 0, imageSize.height > 0 else {
      assertionFailure("Invalid imageSize")
      return .zero
    }
  ...
  private class func imageLayerForItem(
    ...
    guard let srcImage = loadSrcImage(model: model) else {
      assertionFailure("Could not load src image.")
      return nil
    }
  ...
  private class func layerForItem(
    ...
    case .test:
      assertionFailure("Unexpected test item.")
      return nil
    case .stroke:
      guard let strokeItem = item as? ImageEditorStrokeItem else {
        assertionFailure("Item has unexpected type: \(type(of: item)).")
        return nil
      }
  ...
  private class func strokeLayerForItem(
    ...
    if item.isBlur {
      guard
        let blurredImageLayer = blurredImageLayerForItem(
          model: model, transform: transform, viewSize: viewSize)
      else {
        assertionFailure("Failed to retrieve blurredImageLayer")
        return nil
      }

      blurredImageLayer.zPosition = zPositionForItem(
        item: item, model: model, zPositionBase: blurLayerZ)
    ...
    let strokeWidth = ImageEditorStrokeItem.strokeWidth(
      forUnitStrokeWidth: item.unitStrokeWidth,
      dstSize: viewSize)
    ...
    let imageFrame = ImageEditorCanvasView.imageFrame(
      forViewSize: viewSize, imageSize: model.srcImageSizePixels, transform: transform)
    let transformSampleToPoint = { (unitSample: CGPoint) -> CGPoint in
      unitSample.fromUnitCoordinates(viewBounds: imageFrame)
    }

    // Use bezier curves to smooth stroke.
    let bezierPath = UIBezierPath()

    let points = applySmoothing(
      to: unitSamples.map { unitSample in
    ...
    for index in 0..<points.count {
      ...
      if points.count <= 1 {
        ...
        forwardVector = CGPoint.add(previousPointForwardVector, nextPointForwardVector).scaled(
          by: 0.5)
      }
      ...
      if index == 0 {
        ...
        let controlPoint1 = CGPoint.add(
          previousPoint, previousForwardVector.scaled(by: +controlPointFactor))
        let controlPoint2 = CGPoint.add(point, forwardVector.scaled(by: -controlPointFactor))
    ...
    if item.isBlur {
      guard let blurredImageLayer = optionalBlurredImageLayer else {
        assertionFailure("Unexpectedly missing blurredImageLayer")
        return nil
      }
  ...
  private class func zPositionForItem(
    ...
    guard let itemIndex = itemIds.firstIndex(of: item.itemId) else {
      assertionFailure("Couldn't find index of item.")
  ...
  private class func textLayerForItem(
    ...
    let layerSize = CGSize(
      width: textBounds.size.width.rounded(.up), height: textBounds.size.height.rounded(.up))
    layer.frame = CGRect(
      origin: CGPoint(
        x: centerInCanvas.x - layerSize.width * 0.5,
        y: centerInCanvas.y - layerSize.height * 0.5),
      size: layerSize)

    let transform = CGAffineTransform.identity.scaledBy(x: item.scaling, y: item.scaling).rotated(
      by: item.rotationRadians)
    ...
    for index in 0..<points.count {
      ...
      if index == 0 {
        ...
        let smoothedPoint = CGPoint.add(
          point.scaled(by: 1.0 - 2.0 * alpha),
          CGPoint.add(
            lastPoint.scaled(by: alpha),
            nextPoint.scaled(by: alpha)))
  ...
  private func prepareBlurredImage() {
    DispatchQueue.global(qos: .userInteractive).async { [weak self] in
      ...
      guard let clampFilter = CIFilter(name: "CIAffineClamp") else {
        return assertionFailure("Failed to create blur filter")
      }

      // we use a very strong blur radius to ensure adequate coverage of large and small faces
      guard let blurFilter = CIFilter(name: "CIGaussianBlur", parameters: [kCIInputRadiusKey: 25])
      else {
        return assertionFailure("Failed to create blur filter")
      }

      guard let srcImage = ImageEditorCanvasView.loadSrcImage(model: self.model) else {
        return assertionFailure("Could not load src image.")
      }

      let resizedImage = srcImage.resized(maxSizePixels: 300)
      guard let resizedCGImage = resizedImage.cgImage else {
        return assertionFailure("Failed to downsize image for blur")
      }
      ...
      guard let clampOutput = clampFilter.outputImage else {
        return assertionFailure("Failed to clamp src image")
      }
      ...
      guard let blurredOutput = blurFilter.value(forKey: kCIOutputImageKey) as? CIImage else {
        return assertionFailure("Failed to blur clamped image")
      }

      let context = CIContext(options: nil)
      guard let blurredImage = context.createCGImage(blurredOutput, from: inputImage.extent) else {
        return assertionFailure("Failed to create CGImage from blurred output")
      }
  ...
  @objc
  public class func renderForOutput(model: ImageEditorModel, transform: ImageEditorTransform)
    -> UIImage?
  {
    ...
    for item in model.items() {
      guard
        ...
        assertionFailure("Couldn't create layer for item.")
        continue
      }
    ...
    let sortedLayers = layers.sorted(by: { (left, right) -> Bool in
      left.zPosition < right.zPosition
    })
  ...
  public class func locationImageUnit(
    ...
    let imageFrame = self.imageFrame(
      forViewSize: viewBounds.size, imageSize: model.srcImageSizePixels, transform: transform)
    let affineTransformStart = transform.affineTransform(viewSize: viewBounds.size)
    let locationInContent = locationInView.minus(viewBounds.center).applyingInverse(
      affineTransformStart
    ).plus(viewBounds.center)
    let locationImageUnit = locationInContent.toUnitCoordinates(
      viewBounds: imageFrame, shouldClamp: false)
...
extension UIImage {
  ...
  func resized(maxSizePixels: CGFloat) -> UIImage {
    ...
    let renderer = UIGraphicsImageRenderer(
      size: CGSize(width: width, height: height), format: UIGraphicsImageRendererFormat.default())
    // render the image
