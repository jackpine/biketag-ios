...
public enum BinaryFloatingPointComparison<T: BinaryFloatingPoint> {
    case epsilon(T), maxRelative(_ maxRelative: T), either(T, T)
}

public extension BinaryFloatingPoint {
    func isApproxEqual(to other: Self,
                       comparison: BinaryFloatingPointComparison<Self> = .either(Self.leastNonzeroMagnitude, Self.leastNonzeroMagnitude)) -> Bool {
        switch comparison {
        ...
        case let .either(epsilon, maxRelative):
            return isApproxEqualAbsolutely(to: other, epsilon: epsilon) || isApproxEqualRelatively(to: other, maxRelative: maxRelative)
        }
    }

    func isApproxEqualRelatively(to other: Self, maxRelative: Self = Self.leastNonzeroMagnitude) -> Bool {
        let diff = abs(self - other)
        let larger = max(abs(self), abs(other))
        return diff <= larger * maxRelative
    }

    func isApproxEqualAbsolutely(to other: Self, epsilon: Self = Self.leastNonzeroMagnitude) -> Bool {
        let diff = abs(self - other)
        return diff <= epsilon
    }
}
