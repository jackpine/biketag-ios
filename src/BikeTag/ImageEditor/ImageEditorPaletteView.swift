...
public class ImageEditorColor: NSObject {
  ...
  static func == (left: ImageEditorColor, right: ImageEditorColor) -> Bool {
    return left.palettePhase.isApproxEqual(to: right.palettePhase)
  }
}

// MARK: -

private class PalettePreviewView: ImageEditorLayerView {
  ...
  override init() {
    ...
    layoutCallback = { view in
      PalettePreviewView.updateLayers(
        view: view,
  ...
  static func updateLayers(
    ...
    let tangentLength = (circleCenterDistance.squared() - radiusDiff.squared()).squareRoot()
    ...
    let asPoint = CGPoint(x: innerCircleSize.width, y: innerCircleSize.height)
    let circleFrame = CGRect(
      origin: circleCenter.minus(asPoint.scaled(by: 0.5)),
...
public class ImageEditorPaletteView: UIView {
  ...
  private let imageWrapper = ImageEditorLayerView()
  ...
  private func createContents() {
    ...
    imageView.autoPinEdgesToSuperviewEdges(
      with: UIEdgeInsets(top: margin, left: margin, bottom: margin, right: margin))
    imageView.addBorder(colored: .white)
    ...
    selectionView.addBorder(colored: .white)
    ...
    selectionView.autoAlignAxis(toSuperviewAxis: .vertical)

    // There must be a better way to pin the selection view's location,
    // but I can't find it.
    let selectionConstraint = NSLayoutConstraint(
      item: selectionView,
      attribute: .centerY, relatedBy: .equal, toItem: imageWrapper, attribute: .top, multiplier: 1,
      constant: 0)
    ...
    let previewConstraint = NSLayoutConstraint(
      item: previewView,
      attribute: .centerY, relatedBy: .equal, toItem: imageWrapper, attribute: .top, multiplier: 1,
      constant: 0)
  ...
  private func selectColor(atLocationY y: CGFloat) {
    let palettePhase = y.inverseLerp(0, imageView.frame.height, shouldClamp: true)
  ...
  private func value(for palettePhase: CGFloat) -> ImageEditorColor {
    ...
    for color in ImageEditorColor.gradientUIColors {
      if let color0 = prevColor {
        ...
        segments.append(
          GradientSegment(
            color0: color0, color1: color1, palettePhase0: palettePhase0,
            palettePhase1: palettePhase1))
      }
      prevColor = color
    }
    ...
    guard let segment = bestSegment else {
      assertionFailure("Couldn't find matching segment.")
      return ImageEditorColor.defaultColor()
    }
    guard palettePhase >= segment.palettePhase0,
      palettePhase <= segment.palettePhase1
    else {
      assertionFailure("Invalid segment.")
      return ImageEditorColor.defaultColor()
    }
    let segmentPhase = palettePhase.inverseLerp(segment.palettePhase0, segment.palettePhase1)
      .clamped(by: 0...1)
  ...
  private func updateState() {
    ...
    guard let selectionConstraint = selectionConstraint else {
      assertionFailure("Missing selectionConstraint.")
      return
    }
    let selectionY = imageWrapper.frame.height * selectedValue.palettePhase
    selectionConstraint.constant = selectionY

    guard let previewConstraint = previewConstraint else {
      assertionFailure("Missing previewConstraint.")
      return
    }
