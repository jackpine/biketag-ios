...
public class ImageEditorModel: NSObject {
  ...
  public required init(srcImagePath: String) throws {
    ...
    //            throw ImageEditorError.invalidInput
    //        }
    let srcImageSizePixels = try Self.imageSize(
      for: URL(fileURLWithPath: srcImagePath), mimeType: mimeType)
  ...
  class func imageSize(for imageUrl: URL, mimeType _: String) throws -> CGSize {
    ...
    guard let orientationNum = properties.object(forKey: kCGImagePropertyOrientation) as? NSNumber
    else {
  ...
  public func isDirty() -> Bool {
    ...
    return transform
      != ImageEditorTransform.defaultTransform(srcImageSizePixels: srcImageSizePixels)
  }
  ...
  private class func crop(
    ...
    let cropRect = CGRect(
      x: round(unitCropRect.origin.x * srcImageSize.width),
    ...
    guard cropRect.origin.x >= 0,
      ...
      cropRect.origin.y + cropRect.size.height <= srcImageSize.height
    else {
