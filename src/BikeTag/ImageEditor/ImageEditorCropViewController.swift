...
class ImageEditorCropViewController: BaseViewController {
  ...
  public let clipView = ImageEditorLayerView()

  public let croppedContentView = ImageEditorLayerView()
  ...
  private class CropCornerView: ImageEditorLayerView {
  ...
  init(
    delegate: ImageEditorCropViewControllerDelegate,
  ...
  private var cropLockButton: ImageEditorButton?
  ...
  override func loadView() {
    ...
    for cropCornerView in cropCornerViews {
      ...
      default:
        assertionFailure("Invalid crop region: \(String(describing: cropRegion))")
      }
      switch cropCornerView.cropRegion {
  ...
  public func updateNavigationBar() {
    let resetButton = navigationBarButton(
  ...
  private func updateCropLockButton() {
    guard let cropLockButton = cropLockButton else {
      assertionFailure("Missing cropLockButton")
      return
    }
    cropLockButton.setImage(
      imageName: isCropLocked
  ...
  private func updateClipViewLayout() {
    ...
    clipViewConstraints = ImageEditorCanvasView.updateContentLayout(
      transform: transform,
  ...
  private func setCropViewAppearance() {
    ...
    let cornerSize = CGSize(
      width: min(clipView.frame.width * 0.5, ImageEditorCropViewController.desiredCornerSize),
      height: min(clipView.frame.height * 0.5, ImageEditorCropViewController.desiredCornerSize))
    ...
    for cropCornerView in cropCornerViews {
      ...
      cropCornerView.layoutCallback = { view in
        ...
        default:
          assertionFailure("Invalid crop region: \(cropCornerView.cropRegion)")
        }

        shapeLayer.path = bezierPath.cgPath
      }
    }
    cropView.addBorder(colored: .white)
  }
  ...
  private func updateImageLayer() {
    ...
    ImageEditorCanvasView.updateImageLayer(
      imageLayer: croppedImageLayer, viewSize: viewSize, imageSize: model.srcImageSizePixels,
      transform: transform)
    ImageEditorCanvasView.updateImageLayer(
      imageLayer: uncroppedImageLayer, viewSize: viewSize, imageSize: model.srcImageSizePixels,
      transform: transform)
  }
  ...
  private func configureGestures() {
    ...
    let pinchGestureRecognizer = ImageEditorPinchGestureRecognizer(
    ...
    let panGestureRecognizer = ImageEditorPanGestureRecognizer(
  ...
  private class func unitTranslation(
    ...
    let translationUnit = translation.toUnitCoordinates(
      viewSize: viewBounds.size, shouldClamp: false)
  ...
  public func handlePinchGesture(_ gestureRecognizer: ImageEditorPinchGestureRecognizer) {
    ...
    case .changed, .ended:
      guard let gestureStartTransform = gestureStartTransform else {
        assertionFailure("Missing pinchTransform.")
        return
      }
      ...
      let newRotationRadians =
        gestureStartTransform.rotationRadians + gestureRecognizer.pinchStateLast.angleRadians
        - gestureRecognizer.pinchStateStart.angleRadians

      // NOTE: We use max(1, ...) to avoid divide-by-zero.
      //
      // TODO: The clamp limits are wrong.
      let newScaling =
        gestureStartTransform.scaling * gestureRecognizer.pinchStateLast.distance
  ...
  private func handleCropPanGesture(
    ...
    if isCropLocked {
      ...
      if scaleX > scaleY {
        ...
        cropRectangleLocked.size.height =
          cropRectangleNow.width * cropRectangleStart.height / cropRectangleStart.width
      } else {
        cropRectangleLocked.size.height = cropRectangleNow.height
        cropRectangleLocked.size.width =
          cropRectangleNow.height * cropRectangleStart.width / cropRectangleStart.height
      }
  ...
  private func crop(toRect cropRect: CGRect) {
    ...
    let croppedOutputSizePixels = CGSize(
      width: round(transform.outputSizePixels.width * cropRect.width / clipView.frame.width),
      height: round(transform.outputSizePixels.height * cropRect.height / clipView.frame.height))
    ...
    let naiveImageFrameOld = ImageEditorCanvasView.imageFrame(
      forViewSize: transform.outputSizePixels, imageSize: model.srcImageSizePixels,
      transform: naiveTransform)
    let naiveImageFrameNew = ImageEditorCanvasView.imageFrame(
      forViewSize: croppedOutputSizePixels, imageSize: model.srcImageSizePixels,
      transform: naiveTransform)
    ...
    let oldImageFrameCanvas = ImageEditorCanvasView.imageFrame(
    ...
    let oldImageCenterView = oldImageFrameCanvas.center.minus(viewBounds.center).applying(
      oldAffineTransform
    ).plus(viewBounds.center)
    // We transform the "image content center" into the unit coordinates
    // of the crop rectangle.
    let newImageCenterUnit = oldImageCenterView.toUnitCoordinates(
      viewBounds: cropRect, shouldClamp: false)
  ...
  private func handleNormalPanGesture(_ gestureRecognizer: ImageEditorPanGestureRecognizer) {
    ...
    let newLocationView = gestureRecognizer.location(in: clipView)
    let newUnitTranslation = ImageEditorCropViewController.unitTranslation(
      oldLocationView: oldLocationView,
  ...
  private func cropRegion(forGestureRecognizer gestureRecognizer: ImageEditorPanGestureRecognizer)
    -> CropRegion?
  {
    ...
    let left = tolerance
    let top = tolerance
    let right = clipView.frame.width - tolerance
    let bottom = clipView.frame.height - tolerance
  ...
  private func completeAndDismiss() {
    delegate?.cropDidComplete(transform: transform)
  ...
  private func rotateButtonPressed(angleRadians: CGFloat, rotateCanvas: Bool) {
    let outputSizePixels =
      (rotateCanvas
        // Invert width and height.
        ? CGSize(
          width: transform.outputSizePixels.height,
  ...
  @objc func didTapReset(sender _: UIButton) {
    ...
    updateTransform(
      ImageEditorTransform.defaultTransform(srcImageSizePixels: model.srcImageSizePixels))
  }
