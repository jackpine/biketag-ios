...
class ImageEditorCropViewController: BaseViewController {
    ...
    override func loadView() {
        ...
        let rotate90Button = ImageEditorButton(imageName: "image_editor_rotate",
                                               tintColor: UIColor.white) { [weak self] in
            self?.rotate90ButtonPressed()
        }
        let flipButton = ImageEditorButton(imageName: "image_editor_flip",
                                           tintColor: UIColor.white) { [weak self] in
            self?.flipButtonPressed()
        }
        let cropLockButton = ImageEditorButton(imageName: "image_editor_crop_unlock",
                                               tintColor: UIColor.white) { [weak self] in
    ...
    private func setCropViewAppearance() {
        // TODO: Tune the size.
        let cornerSize = CGSize(width: min(clipView.frame.width * 0.5, ImageEditorCropViewController.desiredCornerSize),
    ...
    public func handlePinchGesture(_ gestureRecognizer: ImageEditorPinchGestureRecognizer) {
        ...
        case .changed, .ended:
            ...
            let newScaling = gestureStartTransform.scaling * gestureRecognizer.pinchStateLast.distance / max(1.0, gestureRecognizer.pinchStateStart.distance)
    ...
    private func crop(toRect cropRect: CGRect) {
        ...
        let croppedOutputSizePixels = CGSize(width: round(transform.outputSizePixels.width * cropRect.width / clipView.frame.width),
