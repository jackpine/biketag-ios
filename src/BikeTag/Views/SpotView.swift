...
class SpotView: UIView {
    ...
    didSet {
      ...
      if let spot = spot {
        let nameFormat = NSLocalizedString(
          "posted by %1@\non %2@",
          comment:
            "label affixed to a spot image. Embeds (1) the name of the user who posted the spot and (2) the date/time they posted."
        )
        let timeString = dateFormatter.string(for: spot.createdAt) ?? "?"

        let nameText =
          spot.isCurrentUserOwner
          ? NSLocalizedString("you", comment: "embedded in place of username for your own spots")
          : spot.user.name
        nameLabel.text = String(format: nameFormat, nameText, timeString)

        let numberString = rewardFormatter.string(for: spot.reward) ?? "?"
        let rewardformat = NSLocalizedString(
          "\(User.currencyUnit)%@ reward",
          comment: "label affixed to a spot image. Embeds the reward amount earned if captured.")

        rewardLabel.text = String(format: rewardformat, numberString)

        if spot.isCurrentUserOwner {
          let title = NSLocalizedString(
            "Your Spot! ", comment: "primary button text overlaying spot")
  ...
  required init() {
    ...
    guessSpotButtonViewVCenterConstraint = guessSpotButtonView.autoAlignAxis(
      .vertical, toSameAxisOf: self)

    NotificationCenter.default.addObserver(
      self, selector: #selector(SpotView.updateSpotViewImage), name: Spot.didSetImageNotification,
      object: spot)
  }
